...
static long gen=0;
static unsigned long population=POPULATION;
...
#ifdef GENETIC
	srand(SEED);
	population=coreleft()/NUMBEROFQUEENS;
	for (i=0;i<POPULATION;i++)
	{
	int j;
		if (!gene[i].code)
		{
			gene[i].code=(char *)malloc(NUMBEROFQUEENS);
			if (!gene[i].code)
			{
				population=i-1;
				if (!population) exit (1);
				break;
			}
		}
		...
		{
		int k;
			gene[i].code[j]=(char)(rand()%(HSIZE));
			for (k=0;k<j;k++)
			{
				if (gene[i].code[j]==gene[i].code[k])
				{
					j--;
					break;
				}
...
breed()
{
	...
	if (!memcmp(gene[0].code,gene[population/2].code,HSIZE))
	{
		printf("\nPopulation stagnant\n");
		exit(-1);
	}
...
crossover(GENE *father, GENE *mother, GENE *son, GENE *daughter)
{
	...
	if ((unsigned)rand()<RAND_MAX/10000U)
	{
#ifndef MUTSON
		rand();
		rand();
